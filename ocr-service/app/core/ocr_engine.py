...
def get_ocr():
    global _ocr
    if _ocr is None:
        # Initializing PaddleOCR engine instance
...
def preprocess_image(image_path: str) -> str:
    ...
    # Save processed frames to a temporary directory structure
...
def run_ocr(image_path: str) -> list:
    ...
    # Try processing the original document configuration first
    results = ocr.ocr(image_path, cls=True)
    extracted = parse_results(results)

    # If low confidence or few results, try adaptive preprocessing filters
    if not extracted or get_average_confidence(extracted) < 0.75:
        ...
        try:
            ...
            # Use whichever structure yielded higher character confidence scores
