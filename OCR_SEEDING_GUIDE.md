...
This uses `backend/database/seeders/SeedOcrUiSamplesSeeder.php`. It copies a real scanned image into storage, creates the matching `ApplicationDocument` row, then runs the real OCR job synchronously (no queue worker needed) — same as a real upload.

## ⚠️ TODO before this goes anywhere near production: restore the School ID document-type/face-presence check

`verify_school_id()` in `app/verification/school_id.py` currently does **not** call `check_document_type()` at all — unlike Registration Form and Voter's Certificate, which both still call it normally. That function (`app/upload_checks/document_type_check.py`) is what runs two things at once:
...
This was deliberately removed in commit `1e1d07a` ("fix: remove wrong_document_type auto-reupload for School ID") because it was producing false auto-reupload flags on genuine School IDs — unreliable text-marker matches and missing-photo detection on real ID layouts.

**We're leaving it removed FOR NOW, only to make this batch of seeding/debugging easier** (fewer gates in the way while we're focused on institution/name/school-year matching bugs). This is a reminder, not a decision to keep it out permanently — **once we're done seeding and debugging all 6 schools, someone needs to look at re-adding it** (fix the underlying text-marker/face-detection reliability issue first, don't just flip it back on blind, or it'll reproduce the same false-positive problem that got it removed).
...
2. **Put it at the exact path the seeder expects, OR update the seeder's path constant.** The seeder currently hardcodes:
   ...
   ```
   in [SeedOcrUiSamplesSeeder.php](backend/database/seeders/SeedOcrUiSamplesSeeder.php). This is a machine-specific absolute path — it will NOT exist on your machine as-is. Either:
...
$env:OCR_BATCH='1'; php artisan db:seed --class=SeedOcrUiSamplesSeeder
...
$env:OCR_BATCH='4'; php artisan db:seed --class=SeedOcrUiSamplesSeeder
```
...
```powershell
$env:OCR_SCHOOL='PUP'; $env:OCR_DOC_TYPE='all'; $env:OCR_CASE='6'; php artisan db:seed --class=SeedOcrUiSamplesSeeder
```
...
2. **Register it in the seeder**: add a new block to the `$schools` array in [SeedOcrUiSamplesSeeder.php](backend/database/seeders/SeedOcrUiSamplesSeeder.php):
   ```php
   [
