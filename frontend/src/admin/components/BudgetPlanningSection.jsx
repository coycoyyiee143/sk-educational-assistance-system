...
function BudgetPlanningSection() {
    ...
    return (
        <>
            {/* Budget Allocation Planning — lead with this: it's the tool SK will actually use most,
    since SK's budget is a fixed annual allocation, not applicant demand it needs to predict. */}
            <div className="page-card">
            ...
            {/* Budget Estimation — historical funds tracker.
                Deliberately does NOT show a pass/approval rate here: that would require a total-submissions
                denominator (i.e. rejection tracking), which SK does not have. "Slot Utilization" below is a
                different, legitimate ratio — funded ÷ allocated slots — built entirely from numbers SK has
                always controlled (how many slots they opened, how many they actually funded). */}
            <div className="page-card">
                ...
                {showInfo && (
                    <MethodologyNote>
                        ...
                        spent in past periods. Only the final line below (Next Period estimate) is a
                        forward-looking figure; everything above it is analysis of what already happened,
                        not a prediction.
                        <br /><br />
                    ...
                    <>
                        ...
                        <div className="alert alert-primary py-3 mb-0">
                            ...
                            <span style={{ fontSize: "1.3rem", fontWeight: 600 }}>
                                {formatCurrency(est.projected_budget)}
                            </span>
                            <div className="text-muted small mt-1">
                                Based on ~{est.average_approved_count ?? 0} funded applicants at{" "}
                                {formatCurrency(est.assistance_per_applicant ?? 2000)} each, from past completed periods.
                            </div>
                        </div>
                    </>
                )}
            ...
            {/* Approval Rate Forecast — the one place a real statistical claim is defensible.
                Renamed from "Budget Forecast" to make clear what is actually being forecast:
                the approval rate, not applicant demand. */}
            <div className="page-card">
                ...
                {!forecast?.available ? (
                    ...
                    <>
                        ...
                        <div className="text-muted small mt-3">
                            ...
                            submissions, pooled across {forecast.periods_used} completed period(s).
                        </div>
                    </>
                )}
            </div>
