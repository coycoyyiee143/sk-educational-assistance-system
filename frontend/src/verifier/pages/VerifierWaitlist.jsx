...
function VerifierWaitlist() {
  ...
  async function handlePromote() {
    ...
    try {
      const res = await api.post(
        `/verifier/applications/config/${configId}/promote-waitlist`
      );
      setMessage(res.data.message);
      fetchData();
    } catch (err) {
      setError(
        err.response?.data?.message || "Failed to promote next applicant."
      );
    } finally {
      setPromoting(false);
    }
  ...
  async function handlePromoteAll() {
    ...
    try {
      const res = await api.post(
        `/verifier/applications/config/${configId}/promote-all-waitlist`
      );
      setMessage(res.data.message);
      fetchData();
    } catch (err) {
      setError(
        err.response?.data?.message || "Failed to promote applicants."
      );
    } finally {
  ...
  return (
    <div className="verifier-layout">
      ...
      <div className="verifier-main">
        <VerifierTopbar onMenuOpen={() => setMobileMenuOpen(true)} />
          ...
          <div className="container-fluid">
            <div className="verifier-dashboard-header">
              ...
              <p className="verifier-dashboard-desc">
                Manage applicants who qualified for educational assistance but are currently waiting for an available slot.
              </p>
            </div>
            ...
            <div className="page-card verifier-attention-card">
              <div className="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                <h4 className="verifier-application-list-title mb-0">
                  ...
                  <tbody>
                    {loading ? (
                      ...
                      waitlist.map((app) => (
                        <tr
                          key={app.id}
                          className={app.position === 1 ? "table-warning" : undefined}
                        >
                          <td>
                            {app.position}
                            {app.position === 1 && (
                              <span className="badge bg-warning text-dark ms-2">Next</span>
                            )}
                          </td>
              ...
              {!loading && waitlist.length > 0 && (
                <div className="verifier-table-pagination-bar">
                  ...
                  <div className="verifier-table-pagination-controls">
                    ...
                    <button
                      type="button"
                      className="verifier-table-pagination-page verifier-table-pagination-page-active"
                    >
                      1
                    </button>
