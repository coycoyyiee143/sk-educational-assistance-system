...
function prefillFromLatestAction(latestAction, reasonsByDocType, appStatus) {
  ...
  if (!latestAction || latestAction.action !== "reupload_requested" || !latestAction.reupload_details || appStatus !== "reupload_requested") return base;
...
function VerifierApplicationReview() {
  ...
  const [activeDocumentType, setActiveDocumentType] = useState("voters_certificate");
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const docs = app.documents.filter((doc) => doc.document_type === activeDocumentType);
    const createdUrls = [];
    Promise.all(
      docs.map(async (doc) => {
        try {
          const res = await api.get(`/applications/${app.id}/documents/${doc.id}/file`, { responseType: "blob" });
      ...
      if (!cancelled) setPreviewFiles(Object.fromEntries(entries));
    });
    return () => {
      cancelled = true;
  ...
  const getFlagReasonLabel = (reason) => {
    const labels = {
      ...
      "File uploaded is not the correct document type.": "Incorrect document type",
      "Not issued within the current year.": "Not issued this year",
      "Not a registered voter in Barangay Mamatid.": "Not a registered Mamatid voter",
      "Parent's/guardian's Voter's Certificate could not be validated.": "Parent/guardian voter record invalid",
      "Applicant is not a minor; parent's/guardian's Voter's Certificate not allowed.": "Parent/guardian document not applicable",
      "Name does not match other submitted documents.": "Name mismatch",
  ...
  const checkpointReview = activeChecks.filter((check) => !check.passed).length;
  const checkpointPassed = activeChecks.filter((check) => check.passed).length;
  ...
  const activeDocumentIndex = documentTabs.findIndex((tab) => tab.type === activeDocumentType);
  const nextDocument = activeDocumentIndex < documentTabs.length - 1 ? documentTabs[activeDocumentIndex + 1] : null;
  const previousDocument = activeDocumentIndex > 0 ? documentTabs[activeDocumentIndex - 1] : null;
  ...
  function toggleReason(docType, reasonText) {
    setFlaggedDocs((prev) => {
      ...
      const updated = current.includes(reasonText) ? current.filter((r) => r !== reasonText) : [...current, reasonText];
      ...
      <div className="verifier-main">
        ...
        <section className="page-section">
          <div className="container-fluid">
            ...
            <div className="page-card verifier-review-info-card">
              <div className="verifier-review-info-header">
                <div className="verifier-review-info-header-top">
                  <h4 className="verifier-review-info-title">Applicant Information</h4>
              ...
              <div className="verifier-review-profile-area">
                <div className="verifier-review-profile-main">
                  ...
                  <div className="verifier-review-profile-content">
                    <h5 className="verifier-review-profile-name">{user?.first_name} {user?.last_name}</h5>
                  </div>
                </div>
              </div>
              <div className="verifier-review-information-body">
                <div className="verifier-review-information-column">
                  <p className="verifier-review-info-label">PERSONAL DETAILS</p>
                  <div className="verifier-review-details-grid verifier-review-details-grid-single">
                    {[
                      ["Applicant Full Name", `${user?.first_name} ${user?.last_name}`],
                      ["Date of Birth", profile?.birthdate ?? "—"],
                      ["Email Address", user?.email],
                      ["Mobile Number", user?.mobile_number ?? "—"],
                      ["Residential Address", profile ? [profile.barangay, profile.city, profile.province].filter(Boolean).join(", ") || "—" : "—"],
                    ].map(([label, value]) => (
                      <div className="verifier-review-detail-item" key={label}>
                        <span className="verifier-review-detail-label">{label}</span>
                        <span className="verifier-review-detail-value">{value}</span>
                      </div>
                    ))}
                    {profile?.is_minor && (
                      <div className="verifier-review-detail-item">
                        <span className="verifier-review-detail-label">Guardian (Minor Applicant)</span>
                ...
                <div className="verifier-review-information-column">
                  ...
                  <div className="verifier-review-details-grid verifier-review-details-grid-single">
                    {[
                      ["Institution / School Name", app.school_name],
                      ["Program / Degree", app.course],
                      ["Year Level", app.year_level],
                      ["Student ID", app.student_id_number],
                      ["Current Academic Year", app.configuration?.school_year],
                    ].map(([label, value]) => (
                      <div className="verifier-review-detail-item" key={label}>
                        <span className="verifier-review-detail-label">{label}</span>
                        <span className="verifier-review-detail-value">{value ?? "—"}</span>
            ...
            <div className="page-card verifier-ocr-section" aria-busy={refreshingOcr}>
              <div className="verifier-ocr-section-heading">
                <h4 className="section-title verifier-ocr-section-title">System Document & OCR Integrity Verification</h4>
                <button
                  type="button"
                ...
                >
                  <svg
                    className={`verifier-ocr-refresh-icon ${refreshingOcr ? "verifier-ocr-refresh-icon-spinning" : ""}`}
                    viewBox="0 0 24 24"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                ...
                {documentTabs.map((tab) => {
                  ...
                  return (
                    <button
                      type="button"
                      key={tab.type}
                      className={`verifier-ocr-document-tab ${activeDocumentType === tab.type ? "verifier-ocr-document-tab-active" : ""}`}
                      onClick={() => handleTabClick(tab)}
                      disabled={refreshingOcr}
                    >
                      <span className="verifier-ocr-document-tab-number">{tab.number}</span>
                      <span className="verifier-ocr-document-tab-label">{tab.label}</span>
                      <span className={`verifier-document-tab-status verifier-document-tab-status-${tabStatus.state}`}>
              ...
              <div className="verifier-ocr-refresh-content">
                <div className="verifier-ocr-document-content">
                  {filteredDocuments.map((doc) => {
                    ...
                    const confidence = doc.ocr_result?.confidence_score ? `${(doc.ocr_result.confidence_score * 100).toFixed(1)}%` : "—";
                    const displayedChecks = isLatestVersion ? filteredChecks : relatedChecks;
                    return (
                      <div className="verifier-ocr-review-card mb-4" key={doc.id} style={{ opacity: isLatestVersion ? 1 : 0.75 }}>
                          ...
                          <div className="verifier-ocr-review-header-left">
                            ...
                            <div>
                              <div className="d-flex align-items-center gap-2 flex-wrap">
                                <h6 className="verifier-ocr-review-title">{docLabel}</h6>
                                {isLatestVersion ? (
                                  <span className="badge bg-primary verifier-ocr-badge">Current Version</span>
                                ) : (
                                  <span className="badge bg-secondary verifier-ocr-badge">Archived (v{doc.version})</span>
                                )}
                              </div>
                            </div>
                          </div>
                          <div className="verifier-ocr-review-actions">
                            <button type="button" className="verifier-ocr-file-btn" onClick={() => handleViewFile(doc.id)}>
                            ...
                            {doc.ocr_result?.raw_text && (
                              <button type="button" className="verifier-ocr-file-btn" onClick={() => setActiveRawDocId(activeRawDocId === doc.id ? null : doc.id)}>
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                                  <polyline points="14 2 14 8 20 8" />
                                </svg>
                                {activeRawDocId === doc.id ? "Hide Raw OCR" : "Raw OCR"}
                              </button>
                            )}
                          </div>
                        </div>
                        <div className="verifier-ocr-review-layout">
                          <div className="verifier-ocr-preview-column">
                            <div className="verifier-ocr-preview-heading">Document Preview</div>
                            <div className="verifier-ocr-preview-frame">
                              {!previewFile ? (
                                ...
                                <div className="verifier-ocr-preview-image-wrap" onClick={() => setZoomPreview({ url: previewFile.url, label: docLabel })}>
                                  <img src={previewFile.url} alt={`${docLabel} preview`} className="verifier-ocr-preview-image" />
                                ...
                                <iframe src={previewFile.url} title={`${docLabel} preview`} className="verifier-ocr-preview-iframe" />
                              )}
                            </div>
                            <div className="verifier-ocr-preview-meta">
                              ...
                              <div className="verifier-ocr-preview-meta-row">
                                ...
                                <strong>{formatTimestamp(doc.created_at || doc.updated_at)}</strong>
                              </div>
                              <div className="verifier-ocr-preview-meta-row">
                            ...
                            <div className="verifier-checkpoints-bar">
                              <div className="verifier-checkpoints-info">
                                <div className="verifier-checkpoints-title-row">
                                  <span className="verifier-checkpoints-title">Checkpoints</span>
                                  {activeOverallStatus && (
                                    <span className={`badge verifier-ocr-badge ${activeOverallStatus.class}`}>
                              ...
                              <div className="verifier-checkpoints-controls">
                                <div className="verifier-checkpoints-segments">
                                  <button type="button" className={`verifier-checkpoint-segment ${checkpointFilter === "all" ? "verifier-checkpoint-segment-active" : ""}`} onClick={() => setCheckpointFilter("all")}>All ({checkpointTotal})</button>
                                  <button type="button" className={`verifier-checkpoint-segment ${checkpointFilter === "review" ? "verifier-checkpoint-segment-active" : ""}`} onClick={() => setCheckpointFilter("review")}>Review ({checkpointReview})</button>
                                  <button type="button" className={`verifier-checkpoint-segment ${checkpointFilter === "passed" ? "verifier-checkpoint-segment-active" : ""}`} onClick={() => setCheckpointFilter("passed")}>Passed ({checkpointPassed})</button>
                                </div>
                              </div>
                            </div>
                            {displayedChecks.length > 0 ? (
                              <div className="verifier-ocr-check-list">
                                {displayedChecks.map((check) => (
                                  <div className={`verifier-ocr-check-card ${check.passed ? "verifier-ocr-check-card-passed" : "verifier-ocr-check-card-failed"}`} key={check.id}>
                                    ...
                                    <div className="verifier-ocr-check-values">
                                      <div className="verifier-ocr-check-value-group">
                                        <span className="verifier-ocr-check-value-label">EXTRACTED VALUE</span>
                                        <span className="verifier-ocr-check-value">{check.extracted_value || "not extracted"}</span>
                                      </div>
                                      <div className="verifier-ocr-check-value-group">
                                        <span className="verifier-ocr-check-value-label">EXPECTED VALUE</span>
                                        <span className="verifier-ocr-check-value">{check.expected_value ?? "—"}</span>
                                      </div>
                                    </div>
                                    {check.passed ? (
                                      <div className="verifier-ocr-check-pass-reason">
                                        <span className="verifier-ocr-check-pass-label">Flag Reason:</span>
                                        <span className="verifier-ocr-check-pass-none">None</span>
                                        <span className="verifier-ocr-check-pass-message">• {getPassedCheckMessage(check.check_name)}</span>
                                      </div>
                                    ) : (
                                      <div className="verifier-ocr-check-flag">
                                        <span className="verifier-ocr-check-flag-icon">!</span>
                                        <span className="verifier-ocr-check-flag-label">Flag Reason:</span>
                                        <span className="verifier-ocr-check-flag-text">{check.flag_reason ?? "—"}</span>
                                      </div>
                                    )}
                                  </div>
                                ))}
                              ...
                              <div className="verifier-ocr-empty">
                                {activeChecks.length === 0 ? (
                                  doc.status === "failed" ? (
                                    ...
                                    <div className="verifier-ocr-empty-content">
                                      <span>No execution parameters run against this file configuration.</span>
                                    </div>
                                  )
                                ) : checkpointFilter === "review" ? (
                                  <div className="verifier-ocr-empty-content">
                                    <span>No review checkpoints for this document.</span>
                                  </div>
                                ) : checkpointFilter === "passed" ? (
                                  <div className="verifier-ocr-empty-content">
                                    <span>No passed checkpoints for this document.</span>
                                  </div>
                                ) : (
                                  <div className="verifier-ocr-empty-content">
                                    <span>No checkpoints available for this document.</span>
                                  </div>
                                )}
                              </div>
                            )}
                            {activeRawDocId === doc.id && doc.ocr_result?.raw_text && (
                              <div className="bg-light border rounded p-3 mt-3 text-start verifier-ocr-raw">
                                <h6 className="small fw-bold mb-2 text-dark verifier-ocr-raw-title">PaddleOCR Text Output Stream:</h6>
                                <pre className="mb-0 verifier-ocr-raw-text" style={{ fontSize: "0.75rem", maxHeight: "200px", overflowY: "auto", whiteSpace: "pre-wrap" }}>
                                  {(() => {
                                    try {
                        ...
                        {isLatestVersion && (
                          ...
                          >
                            <summary className="text-danger fw-semibold verifier-ocr-flag-title" style={{ cursor: "pointer" }}>
                              Flag an issue with this document
                            </summary>
                            <div className="verifier-ocr-flag-options">
                              {reasonOptions.map((reason) => (
                                <div className="form-check verifier-ocr-flag-option" key={reason}>
                                  <input
                                  ...
                                  />
                                  <label className="form-check-label small verifier-ocr-check-label" htmlFor={`flag-${doc.document_type}-${reason}`}>
                                    {getFlagReasonLabel(reason)}
                                  </label>
                                  {reason === OTHER && flagState.reasons.includes(OTHER) && (
                                    <input
                ...
                <div className="verifier-review-bottom-actions">
                  <div>
                    {documentDirection === "backward" && previousDocument ? (
                      <button type="button" className="verifier-ocr-file-btn verifier-document-nav-btn" onClick={() => changeDocument(previousDocument, "backward")}>
                        <svg className="verifier-document-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          <polyline points="15 18 9 12 15 6" />
                        </svg>
                        <span>Previous: {previousDocument.label}</span>
                      </button>
                    ) : nextDocument ? (
                      <button type="button" className="verifier-ocr-file-btn verifier-document-nav-btn" onClick={() => changeDocument(nextDocument, "forward")}>
                        <span>Next: {nextDocument.label}</span>
                        <svg className="verifier-document-nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  {["for_review", "pending_prescreening", "reupload_requested"].includes(app.status) && (
                    <button type="button" className="verifier-proceed-action-btn" onClick={handleProceed}>
        ...
        {zoomPreview && (
          <div className="verifier-preview-modal" onClick={() => setZoomPreview(null)}>
            <div className="verifier-preview-modal-content" onClick={(e) => e.stopPropagation()}>
              <button type="button" className="verifier-preview-modal-close" onClick={() => setZoomPreview(null)} aria-label="Close preview">×</button>
              <img src={zoomPreview.url} alt={`${zoomPreview.label} enlarged preview`} className="verifier-preview-modal-image" />
            </div>
          </div>
        )}
        <PanelFooter />
